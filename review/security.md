...
## SEC-113 — `GET /dashboard` serve o `CUTUQUE_TOKEN` no corpo da resposta, sem autenticação; e a escrita do `/board` não exige token nenhum
**Severidade:** R1 se o hub for exposto / R4 (informativo) enquanto ele só escutar na tailnet | **OWASP:** A01:2021 – Broken Access Control + A07:2021 – Identification and Authentication Failures
**Localização:** `hub/internal/server/server.go` — `mux.Handle("GET /dashboard", DashboardHandler(cfg.Token))` (registrada no bloco "Aberta (sem auth)") e `hub/internal/server/dashboard.go:37` (`bytes.Replace(dashboardHTML, tokenPlaceholder, []byte(token), 1)`, onde `tokenPlaceholder = "__CUTUQUE_TOKEN__"`); rotas de escrita do board `POST /board/tasks`, `PATCH /board/tasks/{id}`, `POST /board/tasks/{id}/comments`, todas fora do `requireAuth`
**Detectado:** 2026-08-16 | **Status:** mitigado (2026-08-16, `server.WithPublicMode`) — ver "Fix aplicado"

**Descrição:** o Command Center web precisa do bearer para abrir o WebSocket sem passar `?token=` na URL, e a solução foi injetar o token do hub direto no HTML servido. A rota que serve esse HTML é aberta. As duas decisões, juntas, significam que **um `GET` não autenticado devolve a credencial que abre todas as rotas autenticadas** — `/sessions`, `/machines`, `/ws`, e o PTY. O mesmo raciocínio ("o hub só escuta na interface Tailscale, rede interna, não exposto à internet") está escrito como justificativa em cima das rotas de escrita do board.

**Por que não era bug até agora:** a premissa é verdadeira no deploy de casa. Em prod o `config.Load` resolve `BindAddr` para o IP da tailnet e o hub não escuta em `0.0.0.0`; quem não está na tailnet não alcança porta nenhuma. Nesse mundo, "aberta" quer dizer "aberta para quem já provou pertencer à rede privada".

**Quando vira bug, e foi por isso que apareceu:** a caixa pública de demonstração do review da App Store (Guideline 2.1) roda o MESMO binário num provedor público, com `CUTUQUE_BIND=0.0.0.0` e hostname HTTPS na internet. Ali a premissa deixa de valer inteira e de uma vez: `curl https://<host>/dashboard | grep -o '<token>'` entrega o bearer, e com ele o `/machines/{nome}/pty` — isto é, **um shell interativo aberto para qualquer um que ache a URL**. O impacto concreto não é vazamento de dados (a caixa é descartável e não alcança máquina nenhuma), é uso de recurso: shell exposto vira minerador, e a conta suspensa é a da mantenedora.

**Por que não dá para consertar com `requireAuth`:** `/dashboard` é uma navegação de navegador. O browser não manda header `Authorization` numa navegação, então o middleware de bearer transformaria a página em 401 para o próprio dono. A alternativa seria aceitar `?token=` na query — que grava a credencial no histórico do navegador, no `Referer` das requisições subsequentes e no log de acesso de qualquer proxy no caminho. Trocaria um vazamento sob demanda por um vazamento persistente.

**Fix aplicado (2026-08-16):** `server.WithPublicMode()` — uma `RouterOption` que **não registra** as rotas em vez de tentar autenticá-las. Rota não registrada responde 404: não há o que autenticar e não há o que vazar. Cobre `/dashboard`, `/dashboard-icon.png` (que sai junto por arrumação, não por risco) e as três rotas de escrita do board. Ligada por `CUTUQUE_PUBLIC` (`cmd/hub/main.go`, via `envLigada` — "sim" explícito, desligada por omissão e por valor sem sentido). Ficam intactos `/health` (o healthcheck do provedor depende dele), a leitura do board, `/board-protocol`, `/cutuque`, `/install` e tudo que já exigia token.

**Cobertura:** `hub/internal/server/public_mode_test.go`. Nota sobre `TestDashboardVazaOTokenSemModoPublico`: ele **afirma que o token aparece no HTML** quando o modo está desligado. Não é para proteger o vazamento — é para que, no dia em que o `/dashboard` deixar de injetar o token, o teste quebre e alguém reavalie se esta entrada ainda precisa existir.

**O que continua em aberto:** o modo é uma chave, e chave é coisa que se esquece de ligar. Um hub exposto sem `CUTUQUE_PUBLIC=1` continua vazando o token exatamente como antes. Um passo adiante seria o `config.Load` inferir: `BindAddr` igual a `0.0.0.0` (ou qualquer coisa fora de `127.0.0.1`/faixa CGNAT do Tailscale) com `Env != "prod"` é forte indício de hub exposto, e o hub poderia no mínimo logar um aviso alto, no máximo recusar servir o `/dashboard`. Não foi feito nesta leva para não acoplar detecção de rede a uma decisão de deploy, mas fica registrado como a melhoria que tornaria o buraco impossível em vez de opcional.

## SEC-111 — `Launcher.Answer` não valida que o pending reivindicado é de fato um `AskUserQuestion`, permitindo corromper silenciosamente o `updatedInput` de QUALQUER outra ferramenta pendente (ex.: Bash)
**Severidade:** R2 | **OWASP:** A01:2021 – Broken Access Control (variante "confused deputy"/integridade de estado, não escalonamento de privilégio — mesmo token de sessão já autorizaria `/approve` na mesma sessão)
**Localização:** `hub/internal/launcher/launcher.go` — `Answer` (chama `claimPending` sem checar `p.toolName`) e `buildAnswerResponse` (monta `updatedInput` a partir de `answers`/`originalQuestions(p.input)` incondicionalmente); campo `pending.toolName` (`launcher.go:81`, preenchido em `launcher.go:838`) é escrito mas NUNCA lido em `launcher.go` (confirmado por grep — zero call sites de `.toolName`)
**Detectado:** 2026-07-10 | **Status:** resolved (2026-07-21, verificado por leitura nesta revisão)

**Descrição:** `claimPending` (usado tanto por `respond`/Approve-Deny quanto pelo novo `Answer`) só valida `s.State == StateNeedsYou` e que existe um `pending`+`handle` vivos — nunca valida QUAL ferramenta está pendente. `Answer` assume implicitamente que o pending reivindicado é um `AskUserQuestion` (o próprio comentário do campo `pending.toolName`, linha 76, diz textualmente que ele serve para "distinguir uma pergunta de seleção... de um pedido comum de permissão"), mas o código nunca lê esse campo para impor a distinção.

**PoC confirmado empiricamente** (teste temporário, removido após verificação — não commitado): lançada uma sessão com um pending REAL de `Bash` (`{"command":"touch cutuque.txt","description":"probe"}`, sem `tool_use_id` nem relação com AskUserQuestion), chamando `l.Answer(sid, []session.QuestionAnswer{{Question:"pergunta que não existe", Selected:[]string{"qualquer coisa"}}})` diretamente:
- `Answer` retornou `nil` (sucesso).
- O `control_response` escrito no stdin do CLI foi: `{"type":"control_response","response":{"subtype":"success","request_id":"req-xyz","response":{"behavior":"allow","updatedInput":{"questions":[],"answers":{"pergunta que não existe":"qualquer coisa"}}}}}`.
- Ou seja: o hub respondeu **"allow"** para o `tool_use_id` do Bash pendente, mas **substituiu completamente o `updatedInput` real** (`{"command":"touch cutuque.txt",...}`) por um objeto `{"questions":[],"answers":{...}}` que não tem NENHUMA relação com o schema esperado pela ferramenta Bash — o campo `command` real desaparece do `updatedInput`.

**Como isso é alcançável em produção (sem exigir ator malicioso):** o app decide chamar `/answer` em vez de `/approve`/`/deny` com base no `PendingQuestions` que observou via WS/GET. Como o `Launcher` mantém só UM pending por sessão (mesmo pressuposto arquitetural de "um tool call por vez" já documentado em `patterns.md`), uma janela de corrida entre (a) o CLI emitir um NOVO `control_request` que substitui o pending anterior (ex.: um Bash concomitante a uma pergunta, ou a resolução rápida de uma pergunta seguida de um novo pedido) e (b) o app ainda não ter processado a atualização de estado via WS antes de o usuário tocar "responder", é suficiente para que `Answer` seja chamado sobre um pending que não é mais (ou nunca foi) um `AskUserQuestion`. Um segundo sintoma do MESMO buraco: mesmo quando o toolName está correto, nada valida que o texto de `a.Question` enviado pelo cliente bate com alguma pergunta real em `p.input` — um `Answer` com `question` incorreta (typo, UI desatualizada) constrói `answers:{"<texto errado>":"<label>"}` enquanto `questions` ecoado continua com a pergunta REAL sem resposta correspondente no map, deixando a pergunta de fato feita pelo CLI sem resposta associada (silenciosamente).

**Por que importa:** o CLI provavelmente rejeita/erra ao receber um `updatedInput` fora do schema esperado da ferramenta (best case) — mas o comportamento exato do lado do CLI para esse caso não está confirmado nesta revisão. Independente da reação do CLI, o hub não deveria construir e enviar uma resposta de controle logicamente inconsistente (allow para Bash com o payload de uma pergunta) — isso é um bug de integridade de estado que se manifesta como sessão travada/erro obscuro no CLI, exatamente a classe de sintoma "aparece no CLI/UI, causa é uma camada abaixo" que exigiria uma investigação de Lauren sem esta nota.

**Fix recomendado:**
1. Em `Launcher.Answer`, logo após `claimPending`, checar `if p.toolName != "AskUserQuestion" { l.setPending(id, p); return ErrStaleState }` (devolve o pending intacto — não é um erro de I/O, é reivindicação errada) — simétrico ao já existente `!hasPending || !hasHandle`.
2. Simetricamente, em `respond` (Approve/Deny), rejeitar (ou redirecionar) quando `p.toolName == "AskUserQuestion"` — hoje um Approve sobre uma pergunta pendente ecoa o `input` original sem o campo `answers`, resposta igualmente fora do contrato esperado pelo CLI para essa ferramenta.
3. Validar que cada `a.Question` recebido em `Answer` corresponde a uma das perguntas reais em `p.input` (reusar/generalizar `originalQuestions` para extrair os textos de `question` e comparar) — rejeitar a chamada inteira (não silenciosamente ignorar) se alguma não bater.
4. Cobrir com testes: `TestAnswerRejectsWhenPendingIsNotAskUserQuestion` (usando o mesmo `permissionScript`/Bash já existente) e `TestAnswerRejectsUnknownQuestionText`.

**Atualização (2026-07-10, revisão do front-end/Fase 2):** confirmada uma via de reprodução em produção mais direta do que a corrida descrita acima — a categoria de push `NEEDS_YOU` (`hub/internal/notifier/notifier.go#buildPush`, não tocado por esta leva) sempre expõe as quick actions `APPROVE_ACTION`/`DENY_ACTION` (`app/CutuqueApp/PushManager.swift`, também não tocado por esta leva), mesmo quando `s.PendingQuestions` está preenchido — ou seja, um toque em "Aprovar" direto da tela de bloqueio, sobre uma notificação de uma sessão cujo pending é um `AskUserQuestion`, chama `api.approve(sessionID:)` (`/approve`) e cai exatamente neste bug: hoje (`respond` ainda sem o fix #2 abaixo) isso provavelmente SUCEDE (200) e corrompe silenciosamente o `updatedInput`, sem exigir nenhuma corrida de estado — só um `needs_you` com pergunta pendente e um toque na notificação. Ver também `security.md#SEC-112` (achado irmão no relógio) e `patterns.md#superfície-secundária-não-replica-o-gate-de-autorização-da-superfície-primária`. Recomenda-se tratar o fix #2 desta entrada (rejeitar Approve/Deny quando `p.toolName == "AskUserQuestion"`) como bloqueante para liberar a feature de perguntas de seleção em produção — não só como um "nice to have" — já que o caminho de reprodução via push não depende de nenhum edge case.

**Verificação de resolução (2026-07-21):** confirmado por leitura em `hub/internal/launcher/launcher.go` — `respond` agora rejeita `Approve` sobre pending `AskUserQuestion` (`if allow && p.toolName == toolAskUserQuestion { l.setPending(id, p); return ErrStaleState }`, linha 695, comentário cita "SEC-111" explicitamente) e `Answer` rejeita qualquer pending que NÃO seja `AskUserQuestion` (`if p.toolName != toolAskUserQuestion { l.setPending(id, p); return ErrStaleState }`, linha 724, mesmo comentário). Os fixes #1/#2 recomendados estão aplicados; `validateAnswers` (linha 815) cobre o fix #3 (rejeita resposta que não bate com pergunta real do `p.input`). Não fazia parte do escopo desta leva (nenhum destes trechos aparece no diff revisado) — achado incidental ao ler o contexto de `Answer` para avaliar o novo eco (`buildAnswerEcho`). Fix #2 do lado do PUSH (categoria `NEEDS_YOU` continuar expondo Approve/Deny quick action sobre uma `AskUserQuestion`) segue como consequência do gate agora existir no hub: uma tentativa de Approve por push sobre uma pergunta pendente hoje falha com 409 `stale_state` em vez de corromper o `updatedInput` — o push ainda oferece uma ação que vai falhar (UX subótima, não mais um buraco de integridade), então a recomendação de dar tratamento distinto à categoria `NEEDS_YOU` quando há `PendingQuestions` continua válida como melhoria, mas o risco de SEGURANÇA/integridade original está fechado.

## SEC-112 — Relógio (Watch) responde/cancela uma pergunta de seleção sem checar se a sessão é externa; erro é engolido e reportado como sucesso
**Severidade:** R2 | **OWASP:** A01:2021 – Broken Access Control (gate de autorização implementado numa superfície e não replicado noutra) + falha silenciosa de UX
**Localização:** `app/CutuqueWatch/WatchConnector.swift` (payload de `needsYou` nunca inclui `isExternal`/`external`, só o proxy pré-existente `hasPane`), `app/CutuqueWatch/WatchRootView.swift` (`WatchActionView.questionsSection`, branch `hasQuestions` sem NENHUM gate — ao contrário do branch de Aprovar/Negar, que já usa `!session.hasPane` para o mesmo propósito), `app/CutuqueApp/PhoneWatchRelay.swift` (`case "answer"`: `try? await api.answer(...)` seguido de `replyHandler(["ok": true])` incondicional — mesmo padrão pré-existente de `approve`/`deny`/`reply`, agora replicado para a ação nova)
**Detectado:** 2026-07-10 | **Status:** open

**Descrição:** o iOS guarda corretamente `pendingQuestions`/`permissionPrompt` atrás de `!model.session.isExternal` (`SessionDetailView.swift`) — sessões externas (adotadas via hook/terminal, que o hub não controla pelo canal de controle) nunca oferecem aprovar/negar/responder pelo app, só informam o texto (`externalPromptCard`). O motor do hub (`hub/internal/engine/engine.go:169`, `Apply`) preenche `PendingQuestions` de forma UNIFORME para qualquer sessão em `needs_you` cujo evento seja um `PermissionRequested` com `ToolName=="AskUserQuestion"` — inclusive sessões externas, já que hooks também emitem `PermissionRequested` (é assim que elas entram em `needs_you` hoje). O relógio, porém, não tem nenhuma noção de `isExternal`: o payload de `needsYou` da ponte nunca inclui esse campo, e o novo branch `hasQuestions` de `WatchActionView` não reaplica o proxy `!session.hasPane` que o branch irmão de Aprovar/Negar já usa. Resultado: uma sessão externa com uma `AskUserQuestion` pendente aparece no relógio com a MESMA UI acionável ("Responder"/"Cancelar") de uma sessão lançada pelo app. Como o `Launcher` do hub provavelmente não tem handle/pending de controle para uma sessão desse tipo, a chamada real a `/answer` (ou `/deny`) tende a falhar — mas `PhoneWatchRelay.swift` (`case "answer"`, e igualmente os casos pré-existentes `approve`/`deny`/`reply`) engole o erro via `try?` e sempre responde `["ok": true]`, então o relógio dá o haptic de sucesso, fecha a tela e recarrega como se a resposta tivesse sido entregue — quando a sessão real no terminal continua travada esperando. É exatamente a classe de falha que o próprio `PushManager.swift` já identifica em comentário como "a pior falha aqui é a silenciosa" (ali mitigada com retry + notificação local de falha; aqui, não).

**Fix recomendado:**
1. Propagar `session.isExternal` explicitamente no payload de `needsYou` da ponte (`PhoneWatchRelay.swift`) — não inferir de `hasPane` (que mistura "roda em tmux" com "é externa"; sessões Codex/OpenCode lançadas PELO hub também podem ganhar um pane via fallback tmux, ver log 2026-07-06).
2. Em `WatchRootView.swift`, gatear o branch `hasQuestions` (e os botões "Responder"/"Cancelar" dentro dele) por `!session.isExternal`, espelhando exatamente a regra já usada em `SessionDetailView.swift`.
3. Em `PhoneWatchRelay.swift`, diferenciar sucesso real de falha ao repassar `approve`/`deny`/`answer`/`reply` ao hub — no mínimo refletir o erro real no `replyHandler` (`["ok": false, "error": ...]`) em vez de sempre `true`, e fazer `WatchConnector`/`WatchActionView` sinalizarem falha (mesmo que só um alerta simples) em vez de assumir sucesso.
...
**Detectado:** 2026-07-06 | **Status:** resolved (2026-07-10, verificado por leitura — não fazia parte do escopo desta leva, achado incidental ao revisar `discover.go`/`transcript.go`)

**Verificação de resolução (2026-07-10):** `ocArgs` (`hub/internal/adapter/opencode/target.go:50-53`) agora insere `"--"` antes do prompt (`a = append(a, "--", prompt)`, comentário: "um prompt começando com '-' ... não pode ser lido como flag pelo yargs do opencode (mesmo cuidado do Codex)") — exatamente o fix recomendado no item 1 abaixo. Item 3 (teste de regressão em `ocArgs`) não confirmado nesta leva (fora do escopo revisado, que foi `discover.go`/`transcript.go`); vale checar em `target_test.go` numa próxima passada.

**Descrição (original, 2026-07-06):**
...
**Verificação de resolução (2026-07-10):** `Launcher.resume` (`hub/internal/launcher/launcher.go:698-699`) agora chama `tgt.Start(l.baseCtx, s.ID, s.Cwd, s.Model, "", "", prompt)` — `s.Model` persistido, referenciado explicitamente no comentário da linha 697 ("Reusa o modelo escolhido no launch (persistido em s.Model): o OpenCode exige -m em toda invocação, senão o resume cairia no default (SEC-109)"). Item 3 (teste de regressão em `launcher_test.go` confirmando paridade de modelo entre 1º Start e resume) não confirmado nesta leva (fora do escopo revisado); vale checar numa próxima passada.
...
**Verificação de não-regressão (2026-07-10, leva de paridade discover/transcript do adapter OpenCode):**
1. `opencode/discover.go`: `SSHTarget.Discover` monta `ssh <sshBaseOpts> -- <dest> "python3 -"` — mesma forma exata do `codex/discover.go` (idêntica, sem `id`/argv nenhum, script é constante Go via stdin). Sem input do cliente no comando remoto. **Sem superfície de injeção — igual ao Codex.**
2. `opencode/transcript.go`: `SSHTarget.Transcript` monta `ssh <sshBaseOpts> -- <dest> "python3 - "+agent.SingleQuote(id)` — **código idêntico, char a char, ao `codex/transcript.go`** (mesmo único nível de shell, mesmo `SingleQuote` por valor, mesma dependência de `sessionIDPattern` já validado em `Launcher.Adopt`/`Launcher.Transcript` antes do único call site). `id` nunca é interpolado sem quoting; nunca chega a `Discover` (sem argv). **SEC-101 não regrediu.**
3. Reconfirmado nesta leva (não fazia parte do escopo, mas verificado incidentalmente ao ler `target.go` para contexto de `Discover`/`Transcript`): SEC-109 e SEC-110 (abertos em 2026-07-06, achados 2 e 3 do item anterior) **JÁ FORAM CORRIGIDOS** — ver entradas atualizadas no topo deste arquivo (`ocArgs` agora usa `"--"` antes do prompt; `Launcher.resume` agora propaga `s.Model`). Testes de regressão dedicados (item 3 de cada fix recomendado) não confirmados nesta leva.
